    ...
    <div class="time-bar" :class="[selectPerson.blackOrwhite?'white':'black']">
        ...
        <div class="top-box">
            ...
            <div class="str-box">
                <p class="str">
                    你想活到
                    <b> {{selectPerson.maxAge}} </b>
                    岁，人生已经走了超过{{timeObj.overYears > selectPerson.maxAge ? selectPerson.maxAge : timeObj.overYears}}年了。
					<b v-if="timeObj.overYears > selectPerson.maxAge" style="color:red"> 恭喜，达成目标 </b>
                </p>
                <div class="pgbar">
                    <div
                        class="barMask"
                        :style="{width:`${maxAgeChart > 100 ? '100' : maxAgeChart}%`}"
                        :class="getColorClass(maxAgeChart)"
                    >
                        <span class="num">{{maxAgeChart > 100 ? '100' : maxAgeChart}}%</span>
                    </div>
                </div>
                <p
                    class="str"
        ...
        <div class="bottom-box">
            <p class="str">今天已经过去了{{timeObj.cHours}}小时</p>
            <div class="pgbar">
                <div
                    class="barMask"
                    :style="{width:`${hoursChart}%`}"
                    :class="getColorClass(hoursChart)"
                >
                    <span class="num">{{hoursChart+'%'}}</span>
                </div>
            </div>
            <!-- 0-6 -->
            <p class="str">{{getDay > 0 ? `本周已经过去了${getDay}天多` : `本周过去了不到1天`}}</p>
            <div class="pgbar">
                <div
                    class="barMask"
                    :style="{width:dayChart+'%'}"
                    :class="getColorClass(dayChart)"
                >
                    <span class="num">{{dayChart}}%</span>
                </div>
            </div>
            ...
            <div class="pgbar">
                <div
                    class="barMask"
                    :style="{width:dateChart+'%'}"
					:class="getColorClass(dateChart)"
                >
                    <span class="num">{{dateChart}}%</span>
                </div>
            </div>
            <p class="str">2020年已经过去了{{timeObj.cMonth}}个月</p>
            <div class="pgbar">
                <div class="barMask" :style="{width:monthChart+'%'}"  :class="getColorClass(monthChart)">
                    <span class="num">{{monthChart}}%</span>
                </div>
            </div>
        </div>
        <!-- 设置 -->
...
class Person {
    constructor(
        ...
        this.name = name;
        this.age = age;
        this.txUrl = txUrl || "/img/mypage/tx.jpg";
        // 设置信息
...
export default {
    ...
    created() {
        ...
        if (ls.read() && ls.read().length) {
            ...
            // 默认数据
            let pereson = new Person(1, "小明", "1993-09-09");
    ...
    computed: {
        ...
        maxAgeChart() {
            return Math.floor(
                (this.timeObj.overYears / this.selectPerson.maxAge) * 100
            );
        },
        hoursChart() {
            return Math.floor(((this.timeObj.cHours + this.timeObj.cM/60) / 24) * 100);
        },
        dayChart() {
            return Math.floor((this.getDay+(this.hoursChart/100))/ 7 * 100);
		},
		dateChart() {
			return Math.round((this.timeObj.cData-1+(this.hoursChart/100))/this.timeObj.days*100);
		},
		monthChart() {
			return Math.floor((this.timeObj.cMonth+this.dateChart/100)/12*100);
		}
    },
    watch: {
        selectPersonId() {
...
function computed(birthday) {
    ...
    return { overYears, cMonth, cData, cDay, cHours, cM, longDay, weekEnd, eat, days };
}
